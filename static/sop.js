    ...
    function renderWorkflow() {
      ...
      phaseListEl.innerHTML = weeklyPlan.map((week) => {
        ...
        const canCollapseWeek = !!anchor && week.id < currentWeekId;
        const isWeekCollapsed = canCollapseWeek ? collapsedWorkflowWeeks[week.id] !== false : false;
        return `
          <section class="phase">
    ...
    function renderOverview() {
      ...
      overviewGridEl.innerHTML = [
        { label: "Active", value: active.length, desc: "当前还在带新中的主播" },
        { label: "Warning", value: warning, desc: "超过 4 周标准周期的主播" },
        { label: "Done", value: completed, desc: "已完成 4 周标准计划的主播" },
        { label: "Avg Days", value: averageDays, desc: "当前主播平均培训天数" }
      ].map((item) => `
    ...
    function renderSummary() {
      ...
      summaryGridEl.innerHTML = anchors
        ...
        .map((anchor) => `
          <article class="summary-card ${expandedSummaryAnchor === anchor.anchorName ? "expanded" : ""}" data-summary-anchor="${escapeHtml(anchor.anchorName)}">
            ...
            <div class="summary-body">
              <div class="summary-cell">
                <span class="mini">Current Week</span>
                <strong>第 ${anchor.currentWeek} 周</strong>
              </div>
              <div class="summary-cell">
                <span class="mini">Training Days</span>
                <strong>${getDaysInTraining(anchor)} 天</strong>
              </div>
              <div class="summary-cell">
                <span class="mini">Current Blocker</span>
      ...
      planGridEl.innerHTML = weeklyPlan.map((week) => `
        <section class="plan-card">
          ...
          <div class="plan-card-body">
            <ul>
              ${week.actions.map((action) => `<li><strong>${action.title}</strong>：${action.desc}</li>`).join("")}
            </ul>
          </div>
        </section>
      `).join("");
