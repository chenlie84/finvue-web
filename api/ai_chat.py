...
async def list_sessions(session: dict = Depends(security.require_login)) -> list[dict[str, Any]]:
    """获取用户的会话列表"""
...
async def create_session(request: Request, session: dict = Depends(security.require_login)) -> dict[str, Any]:
    """创建新会话"""
...
async def update_session(session_id: str, request: Request, session: dict = Depends(security.require_login)) -> dict[str, Any]:
    """更新会话"""
    body = await request.json()
    username = session.get("username")
...
async def delete_session(session_id: str, session: dict = Depends(security.require_login)) -> dict[str, Any]:
    """删除会话"""
    username = session.get("username")
...
async def get_messages(session_id: str, session: dict = Depends(security.require_login)) -> list[dict[str, Any]]:
    """获取会话消息"""
    username = session.get("username")
...
async def send_message(request: Request, session: dict = Depends(security.require_login)) -> dict[str, Any]:
    """发送消息并获取AI回复"""
    body = await request.json()
    username = session.get("username")
...
async def stream_message(request: Request, session: dict = Depends(security.require_login)) -> StreamingResponse:
    """流式发送消息"""
    body = await request.json()
    username = session.get("username")
...
async def list_prompts(session: dict = Depends(security.require_login)) -> list[dict[str, Any]]:
    """获取提示词模板列表"""
    username = session.get("username")
    sql = """
...
async def create_prompt(request: Request, session: dict = Depends(security.require_login)) -> dict[str, Any]:
    """创建提示词模板"""
    body = await request.json()
    username = session.get("username")
...
async def update_prompt(prompt_id: int, request: Request, session: dict = Depends(security.require_login)) -> dict[str, Any]:
    """更新提示词模板"""
    body = await request.json()
    username = session.get("username")
...
async def delete_prompt(prompt_id: int, session: dict = Depends(security.require_login)) -> dict[str, Any]:
    """删除提示词模板"""
    username = session.get("username")
...
async def list_knowledge(session: dict = Depends(security.require_login)) -> list[dict[str, Any]]:
    """获取知识库列表"""
    username = session.get("username")
    sql = """
...
async def upload_knowledge(
    ...
    session: dict = Depends(security.require_login)
) -> dict[str, Any]:
    """上传知识库文件"""
    username = session.get("username")
...
async def delete_knowledge(knowledge_id: int, session: dict = Depends(security.require_login)) -> dict[str, Any]:
    """删除知识库"""
    username = session.get("username")
...
async def get_knowledge_content(knowledge_id: int, session: dict = Depends(security.require_login)) -> dict[str, Any]:
    """获取知识库详细内容"""
    username = session.get("username")
...
async def chat_completion(request: Request, session: dict = Depends(security.require_login)) -> dict[str, Any]:
    """OpenAI 兼容的 Chat API"""
    body = await request.json()
    username = session.get("username")
