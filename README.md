...
DAILY_MARKET_REVIEW_BOARD_FETCH_SECONDS=110
```

如果 TuShare Token 没有 `ths_daily` 同花顺板块日线权限，脚本会自动降级使用东方财富公共板块数据。`DAILY_MARKET_REVIEW_BOARD_FETCH_SECONDS` 用于限制公共板块 K 线抓取预算，预算耗尽时会使用已抓到的数据继续生成，避免整份复盘卡到 API 总超时。
